...
#define MMU_CMD_TIMEOUT 60000ul // 10s timeout for mmu commands (except P0,C0)
#define MMU_C0_TIMEOUT 10000ul  // Timeout for C0 command: 10seconds
#define MMU_C0_TRY 6            // Try 10 C0 : 60 ( 6 x 10s) seconds
...
char MMU2::rx_buffer[2048], MMU2::tx_buffer[2048];
int8_t MMU2::nb_c0_try=0;
long int last_time_reset;
...
static constexpr E_Step load_to_nozzle_sequence_start_C0[] PROGMEM = {MMU2_LOAD_TO_NOZZLE_SEQUENCE_START_C0};
static constexpr E_Step load_to_nozzle_sequence_after_C0[] PROGMEM = {MMU2_LOAD_TO_NOZZLE_SEQUENCE_AFTER_C0};
#endif  //MMU2_C0_LOAD_SEQUENCE
...
void MMU2::mmu_loop()
{
  ...
  case -1:
    ...
    else if (millis() > (last_time_reset+3000000))
    {
  ...
  case 3: // response to mmu commands
    if (rx_ok())
    {
      ...
      nb_c0_try = 0;
    }else if ((nb_c0_try < MMU_C0_TRY)&&(last_cmd == MMU_CMD_C0))
    {
      if (ELAPSED(millis(), last_request + MMU_C0_TIMEOUT)){
        // continue loading
        DEBUG_ECHOLNPGM("MMU <= 'C0'");
        tx_str_P(PSTR("C0\n"));
        state = 3; // wait for response
...
#if ENABLED(MMU2_C0_LOAD_SEQUENCE)
void MMU2::load_to_nozzle_start_C0()
{
  if (!enabled)
    return;
  safe_delay(1000);
  execute_extruder_sequence((const E_Step *)load_to_nozzle_sequence_start_C0, COUNT(load_to_nozzle_sequence_start_C0));
}

void MMU2::load_to_nozzle_after_C0()
{
  if (!enabled)
    return;
  execute_extruder_sequence((const E_Step *)load_to_nozzle_sequence_after_C0, COUNT(load_to_nozzle_sequence_after_C0));
}
#endif 

#if ENABLED(MMU2_EXTRA_EJECT_LENGHT)
bool MMU2::force_eject_filament()
{
  ...
  enable_E0();
  current_position.e -= MMU2_FILAMENTCHANGE_EJECT_FORCE_FEED;
  line_to_current_position(MMM_TO_MMS(MMU2_FILAMENTCHANGE_EJECT_FORCE_SPEED));
  planner.synchronize();
...
bool MMU2::eject_filament(uint8_t index, bool recover)
{
  ...
  line_to_current_position(MMM_TO_MMS(MMU2_FILAMENTCHANGE_EJECT_SPEED));
  planner.synchronize();
